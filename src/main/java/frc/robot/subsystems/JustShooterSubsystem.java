...
public class JustShooterSubsystem extends SubsystemBase {
  ...
  private static final double kMinShooterOutput = -1.0;
  private static final double kMaxShooterOutput = 1.0;
  ...
  public void setShooterSpeed(double speed) {
    double clamped = Math.max(kMinShooterOutput, Math.min(kMaxShooterOutput, speed));
    JustShooterMotor.set(clamped);
  }
  ...
  public Command stopJustShooterCommand() {
    return Commands.runOnce(() -> setShooterSpeed(-0.2), this); // stops the shooter motor
  }

  // AUTO COMMAND FOR PATH PLANNER TO SHOOT FUEL THAT ARE ALREADY LOADED.
  public Command autoJustShooterCommand() {
    return Commands.sequence(
    Commands.runOnce(() -> setShooterSpeed(-0.7), this), // Start shooter at 100% speed
        Commands.waitSeconds(5.5), // Wait for 5.5 seconds
    Commands.runOnce(() -> setShooterSpeed(0), this));
  }
  // AUTO COMMAND FOR PATH PLANNER TO REVERSE THE SHOOTER
  public Command autoReverseJustShooterCommand() {
    return Commands.sequence(
        Commands.runOnce(() -> setShooterSpeed(1), this), // Start shooter at 100% speed in reverse
